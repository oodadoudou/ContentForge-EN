...
def check_epub_needs_processing(epub_path, cc):
    ...
    Returns: (needs_layout_change, needs_character_conversion)
    """
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 1. Check whether layout conversion is needed (vertical-rl -> horizontal-tb)
...
def find_opf_file(temp_dir):
    """Find the path to the .opf file in the extracted directory."""
...
def modify_opf_file(opf_path, cc, do_layout, do_chars):
    """Modify the .opf file, converting layout and/or text as needed."""
    ...
    try:
        ...
        print(f"  - [Error] Error modifying OPF file: {e}")

def modify_content_files(temp_dir, cc, do_layout, do_chars):
    """Modify content files, converting layout and/or text as needed."""
    for root_dir, _, files in os.walk(temp_dir):
        for filename in files:
            ...
            try:
                ...
                if content != original_content:
                    with open(file_path, 'w', encoding='utf-8') as f:
                        f.write(content)
                    print(f"  - [Modify] Updated: {os.path.relpath(file_path, temp_dir)}")
            except Exception as e:
...
def process_epub_file(epub_path, output_dir, cc):
    """Process a single EPUB file: detect and convert as needed."""
    ...
    if not needs_layout and not needs_chars:
        print("  - [Skip] File does not require conversion.")
        return

    print(f"  - [Task] Detected requirement: {'Layout conversion ' if needs_layout else ''}{'Text conversion' if needs_chars else ''}")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        try:
            ...
            opf_path = find_opf_file(temp_dir)
            if not opf_path:
                print("  - [Error] Could not find .opf configuration file!")
                return
...
def process_txt_file(txt_path, output_dir, cc):
    """Process a single TXT file, converting Traditional to Simplified Chinese only."""
    ...
    try:
        ...
        if content == converted_content:
            print("  - [Skip] TXT file does not require conversion.")
            return
        ...
        print(f"  - [Error] Error processing TXT file: {e}")
...
def initialize_opencc():
    ...
    try:
        ...
        try:
            ...
            print("Both standard and fallback modes failed. This may be due to incomplete installation or permission issues.", file=sys.stderr)
            print(f"\n- Standard mode error: {e_simple}", file=sys.stderr)
            print(f"- Fallback mode error: {e_fallback}", file=sys.stderr)
            print("\nTry fully reinstalling: pip uninstall opencc-python-reimplemented -y && pip install opencc-python-reimplemented", file=sys.stderr)
            sys.exit(1)
